# -*- mode: python ; coding: utf-8 -*-
"""
黃金跟單系統 - PyInstaller 打包配置
執行方式: python -m PyInstaller build.spec --noconfirm
"""
import sys
from pathlib import Path
from PyInstaller.utils.hooks import collect_data_files

block_cipher = None

# RapidOCR 模型檔案
rapid_datas = []
rapid_hidden = []
try:
    rapid_datas = collect_data_files('rapidocr', include_py_files=False)
    rapid_hidden = [
        'rapidocr',
        'rapidocr.main',
        'rapidocr.cal_rec_boxes',
        'rapidocr.cal_rec_boxes.main',
        'rapidocr.ch_ppocr_cls',
        'rapidocr.ch_ppocr_cls.main',
        'rapidocr.ch_ppocr_cls.utils',
        'rapidocr.ch_ppocr_det',
        'rapidocr.ch_ppocr_det.main',
        'rapidocr.ch_ppocr_det.utils',
        'rapidocr.ch_ppocr_rec',
        'rapidocr.ch_ppocr_rec.main',
        'rapidocr.ch_ppocr_rec.typings',
        'rapidocr.ch_ppocr_rec.utils',
        'rapidocr.inference_engine',
        'rapidocr.inference_engine.base',
        'rapidocr.inference_engine.onnxruntime',
        'rapidocr.inference_engine.onnxruntime.main',
        'rapidocr.inference_engine.onnxruntime.provider_config',
        'rapidocr.utils',
        'rapidocr.utils.download_file',
        'rapidocr.utils.load_image',
        'rapidocr.utils.log',
        'rapidocr.utils.output',
        'rapidocr.utils.parse_parameters',
        'rapidocr.utils.process_img',
        'rapidocr.utils.to_json',
        'rapidocr.utils.to_markdown',
        'rapidocr.utils.typings',
        'rapidocr.utils.utils',
        'rapidocr.utils.vis_res',
    ]
except Exception:
    print("Warning: RapidOCR not found, skipping...")

a = Analysis(
    ['copy_trader/main_gui.py'],
    pathex=['copy_trader'],
    binaries=[],
    datas=[
        ('mt5_ea/MT5_File_Bridge_Enhanced.mq5', 'mt5_ea'),
    ] + rapid_datas,
    hiddenimports=[
        'PySide6.QtCore', 'PySide6.QtGui', 'PySide6.QtWidgets',
        'PySide6.QtSvg',
        'qasync',
        'win32gui', 'win32ui', 'win32con', 'win32api',
        'PIL', 'PIL.Image', 'PIL.ImageGrab',
        'imagehash',
        'rapidocr', 'onnxruntime',
        'groq', 'httpx', 'httpx._transports', 'httpx._transports.default',
        'httpcore', 'httpcore._async', 'httpcore._sync',
        'anyio', 'anyio._backends', 'anyio._backends._asyncio',
        'sniffio', 'distro', 'h11',
        # copy_trader 內部模組
        'config', 'app',
        'gui', 'gui.main_window', 'gui.theme', 'gui.strings',
        'gui.backend_bridge', 'gui.mt5_data_reader', 'gui.log_handler', 'gui.tray',
        'gui.widgets', 'gui.widgets.dashboard', 'gui.widgets.settings_panel',
        'gui.widgets.positions_table', 'gui.widgets.trade_history',
        'gui.widgets.log_viewer', 'gui.widgets.tutorial',
        'signal_capture', 'signal_capture.screen_capture', 'signal_capture.ocr',
        'signal_parser', 'signal_parser.keyword_filter', 'signal_parser.groq_parser',
        'signal_parser.groq_vision_parser', 'signal_parser.gemini_vision_parser',
        'google.genai', 'google.genai.types',
        'signal_parser.parser', 'signal_parser.prompts', 'signal_parser.regex_parser',
        'trade_manager', 'trade_manager.manager',
    ] + rapid_hidden,
    hookspath=[],
    runtime_hooks=[],
    excludes=[
        # Python 標準庫不需要的
        'tkinter', 'unittest', 'xmlrpc', 'pydoc',
        # 科學計算/開發工具
        'matplotlib', 'scipy', 'jupyter', 'notebook', 'IPython', 'pytest',
        # OCR 不需要的
        'winsdk',
        'paddleocr', 'paddle', 'paddlepaddle', 'rapidocr_onnxruntime',
        # 不需要的 transitive deps
        'pywt', 'PyWavelets',
        # PySide6 不需要的模組
        'PySide6.QtQuick', 'PySide6.QtQml', 'PySide6.QtQmlModels',
        'PySide6.QtPdf', 'PySide6.QtOpenGL', 'PySide6.QtNetwork',
        'PySide6.QtVirtualKeyboard', 'PySide6.QtMultimedia',
        'PySide6.QtWebEngine', 'PySide6.QtWebEngineCore',
        'PySide6.Qt3DCore', 'PySide6.QtBluetooth', 'PySide6.QtNfc',
        'PySide6.QtPositioning', 'PySide6.QtRemoteObjects',
        'PySide6.QtSensors', 'PySide6.QtSerialPort', 'PySide6.QtTest',
    ],
    cipher=block_cipher,
)

# Remove unnecessary large binaries
_exclude_bins = [
    # PySide6/Qt unused modules
    'opengl32sw', 'qt6quick', 'qt6qml', 'qt6pdf', 'qt6opengl',
    'qt6virtualkeyboard', 'qt6qmlmodels', 'qt6network',
    'qdirect2d', 'qtuiotouchplugin', 'qtvirtualkeyboard',
    # OpenCV unused
    'opencv_videoio_ffmpeg',
    # pywin32 - keep pythonwin/win32ui + mfc140u (needed for screen capture)
    # (Shapely needed by RapidOCR - do not exclude)
    # PIL - AVIF codec not needed for screenshots
    '_avif',
]
a.binaries = [b for b in a.binaries if not any(x in b[0].lower() for x in _exclude_bins)]

# Remove unnecessary data files
_exclude_datas = [
    'pywt',
]
a.datas = [d for d in a.datas if not any(x in d[0] for x in _exclude_datas)]

pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)

exe = EXE(
    pyz,
    a.scripts,
    [],
    exclude_binaries=True,
    name='黃金跟單系統',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=True,
    console=False,  # 不顯示命令列視窗
)

coll = COLLECT(
    exe,
    a.binaries,
    a.datas,
    strip=False,
    upx=True,
    name='黃金跟單系統',
)
