...
判斷順序：**`?lang=` 參數 → 使用者上次的選擇 → 繁中**。

**刻意不看 `navigator.language`。** 這個站的客群在台灣，而台灣不少人的系統與
瀏覽器是設英文的 —— 跟著瀏覽器語言走的話，這些人第一次進來會看到英文版，
那是錯的對象看到錯的語言。

英文版留給主動切換的人，切了會記在 `localStorage`，之後回來直接是英文
（使用者的明確選擇要贏過任何自動判斷）。分享連結想指定語言就加 `?lang=en`。
